...
public class NormalTransformer implements IClassTransformer {
    ...
    public NormalTransformer() {
        ...
        if (NormalLoadingPlugin.isClient) {
            ...
            if (squashBakedQuads) {
                ...
                for (String classThatExtendBakedQuad : NormalConfig.instance.classesThatExtendBakedQuad) {
            ...
            if (NormalConfig.instance.optimizeSomeRendering) {
                addTransformation("net.minecraft.client.renderer.RenderGlobal", bytes -> fixEnumFacingValuesClone(bytes, NormalLoadingPlugin.isDeobf ? "setupTerrain" : "func_174970_a"));
            }
    ...
    private byte[] extendSupportingBakedQuadInstead(byte[] bytes) {
        ...
        if (node.superName.equals("net/minecraft/client/renderer/block/model/BakedQuad")) {
            node.superName = "mirror/normalasm/bakedquad/SupportingBakedQuad";
        }
            ...
            while (iter.hasNext()) {
                ...
                if (method.name.equals("<init>") && instruction instanceof MethodInsnNode) {
                    ...
                    if (methodNode.getOpcode() == INVOKESPECIAL && methodNode.owner.equals("net/minecraft/client/renderer/block/model/BakedQuad")) {
                        methodNode.owner = "mirror/normalasm/bakedquad/SupportingBakedQuad";
                    }
                    ...
                    if (fieldNode.owner.equals("net/minecraft/client/renderer/block/model/BakedQuad")) {
                        if (fieldsToLookOutFor.contains(fieldNode.name)) {
                            fieldNode.owner = "mirror/normalasm/bakedquad/SupportingBakedQuad";
                        }
                    }
                }
            }
            ...
            if (method.name.equals("<init>") && method.desc.equals("(I[Ljava/lang/String;)V")) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction instanceof MethodInsnNode && instruction.getOpcode() == INVOKEVIRTUAL && ((MethodInsnNode) instruction).name.equals("toLowerCase")) {
                        NormalLogger.instance.info("Injecting calls in {}{} to canonicalize strings", node.name, method.name);
                        iter.previous();
                        iter.previous(); // Move to GETSTATIC
                        iter.remove(); // Remove GETSTATIC
                        iter.next(); // Replace INVOKEVIRTUAL with INVOKESTATIC
                        iter.set(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/api/NormalStringPool", "lowerCaseAndCanonicalize", "(Ljava/lang/String;)Ljava/lang/String;", false));
                    }
                }
            }
        }
            ...
            if (method.name.equals("<clinit>")) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction instanceof TypeInsnNode && instruction.getOpcode() == NEW) {
                        ...
                        NormalLogger.instance.info("Canonizing {} IConditions", bool ? "TRUE" : "FALSE");
                        iter.remove(); // Remove NEW
                        iter.next();
                        iter.remove(); // Remove DUP
                        iter.next();
                        iter.remove(); // Remove INVOKESPECIAL
                        iter.add(new FieldInsnNode(GETSTATIC, "mirror/normalasm/client/models/conditions/CanonicalConditions", bool ? "TRUE" : "FALSE", "Lnet/minecraft/client/renderer/block/model/multipart/ICondition;"));
                    }
                }
            }
        }
    ...
    private byte[] canonicalPredicatedConditions(byte[] bytes, boolean or) {
        ...
        final String getPredicate = NormalLoadingPlugin.isDeobf ? "getPredicate" : "func_188118_a";

        for (MethodNode method : node.methods) {
            if (method.name.equals(getPredicate)) {
                final String conditions = NormalLoadingPlugin.isDeobf ? "conditions" : or ? "field_188127_c" : "field_188121_c";
                NormalLogger.instance.info("Transforming {}::getPredicate to canonicalize different IConditions", node.name);
                ...
                method.instructions.add(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/client/models/conditions/CanonicalConditions", or ? "orCache" : "andCache", "(Ljava/lang/Iterable;Lnet/minecraft/block/state/BlockStateContainer;)Lcom/google/common/base/Predicate;", false));
    ...
    private byte[] canonicalPropertyValueConditions(byte[] bytes) {
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals(getPredicate)) {
                NormalLogger.instance.info("Transforming {}::getPredicate to canonicalize different PropertyValueConditions", node.name);
                method.instructions.clear();
                method.instructions.add(new VarInsnNode(ALOAD, 1));
                method.instructions.add(new VarInsnNode(ALOAD, 0));
                method.instructions.add(new FieldInsnNode(GETFIELD, node.name, NormalLoadingPlugin.isDeobf ? "key" : "field_188125_d", "Ljava/lang/String;"));
                method.instructions.add(new VarInsnNode(ALOAD, 0));
                method.instructions.add(new FieldInsnNode(GETFIELD, node.name, NormalLoadingPlugin.isDeobf ? "value" : "field_188126_e", "Ljava/lang/String;"));
                method.instructions.add(new FieldInsnNode(GETSTATIC, node.name, NormalLoadingPlugin.isDeobf ? "SPLITTER" : "field_188124_c", "Lcom/google/common/base/Splitter;"));
                method.instructions.add(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/client/models/conditions/CanonicalConditions", "propertyValueCache", "(Lnet/minecraft/block/state/BlockStateContainer;Ljava/lang/String;Ljava/lang/String;Lcom/google/common/base/Splitter;)Lcom/google/common/base/Predicate;", false));
                method.instructions.add(new InsnNode(ARETURN));
    ...
    private byte[] cacheMultipartBakedModels(byte[] bytes) {
        ...
        final String makeMultipartModel = NormalLoadingPlugin.isDeobf ? "makeMultipartModel" : "func_188647_a";

        for (MethodNode method : node.methods) {
            if (method.name.equals(makeMultipartModel)) {
                NormalLogger.instance.info("Transforming {}::makeMultipartModel", node.name);
                final String builderSelectors = NormalLoadingPlugin.isDeobf ? "builderSelectors" : "field_188649_a";
                method.instructions.clear();
                method.instructions.add(new VarInsnNode(ALOAD, 0));
                method.instructions.add(new FieldInsnNode(GETFIELD, node.name, builderSelectors, "Ljava/util/Map;"));
                method.instructions.add(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/client/models/MultipartBakedModelCache", "makeMultipartModel", "(Ljava/util/Map;)Lnet/minecraft/client/renderer/block/model/MultipartBakedModel;", false));
                method.instructions.add(new InsnNode(ARETURN));
            }
        }
    ...
    private byte[] removeValuesArrayFromRegistrySimple(byte[] bytes) {
        ...
        final String values = NormalLoadingPlugin.isDeobf ? "values" : "field_186802_b";
    ...
    private byte[] removeDupeMapFromSoundRegistry(byte[] bytes) {
        ...
        final String soundRegistry = NormalLoadingPlugin.isDeobf ? "soundRegistry" : "field_148764_a";
                    ...
                    if (instruction instanceof MethodInsnNode) {
                        ...
                        if (methodInstruction.owner.equals("com/google/common/collect/Maps")) {
                            ...
                            if (!isExperienceList) {
                                ...
                                iter.add(new FieldInsnNode(GETSTATIC, "mirror/normalasm/api/HashingStrategies", "FURNACE_INPUT_HASH", "Lit/unimi/dsi/fastutil/Hash$Strategy;"));
                                ...
                                iter.add(new FieldInsnNode(GETFIELD, "net/minecraft/item/crafting/FurnaceRecipes", NormalLoadingPlugin.isDeobf ? "experienceList" : "field_77605_c", "Ljava/util/Map;"));
    ...
    private byte[] removeInstancedRandom(byte[] bytes) {
        ...
        for (MethodNode method : node.methods) {
            ...
            } else if (method.name.equals(NormalLoadingPlugin.isDeobf ? "cloneEntry" : "func_148720_g")) {
                ListIterator<AbstractInsnNode> iter = method.instructions.iterator();
                while (iter.hasNext()) {
                    AbstractInsnNode instruction = iter.next();
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == INVOKESTATIC) {
                        ...
                        if (methodInsnNode.name.equals("values") && methodInsnNode.desc.equals("()[Lnet/minecraft/util/EnumFacing;")) {
                            NormalLogger.instance.info("Transforming EnumFacing::values() to EnumFacing::VALUES in {}", node.name);
                            iter.set(new FieldInsnNode(GETSTATIC, "net/minecraft/util/EnumFacing", NormalLoadingPlugin.isDeobf ? "VALUES" : "field_82609_l", "[Lnet/minecraft/util/EnumFacing;"));
                        }
                    }
                }
            }
    ...
    private byte[] removePackageField(byte[] bytes) {
        ...
        try {
            Map<String, Package> packages = (Map<String, Package>)  NormalReflector.getField(ClassLoader.class, "packages").get(Launch.classLoader);
            Set<String> packageStrings = packages.keySet();
            packageStrings.forEach(NormalStringPool::canonicalize);
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals("<init>") && method.desc.equals("(Ljava/io/File;Ljava/io/File;Lnet/minecraftforge/fml/common/discovery/ContainerType;ZZ)V")) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == PUTFIELD) {
                        ...
                        if (fieldNode.name.equals("packages")) {
                            ...
                            iter.previous();
                            iter.previous();
                            iter.set(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/core/NormalHooks", "createHashSet", "()Lit/unimi/dsi/fastutil/objects/ObjectOpenHashSet;", false));
                            break;
                        }
                    }
                }
            } else if (method.name.equals("addClassEntry")) { // see: NormalHooks::modCandidate$override$addClassEntry
                method.instructions.clear();
                method.instructions.add(new VarInsnNode(ALOAD, 0));
                method.instructions.add(new VarInsnNode(ALOAD, 1));
                method.instructions.add(new VarInsnNode(ALOAD, 0));
                ...
                method.instructions.add(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/core/NormalHooks", "modCandidate$override$addClassEntry", "(Lnet/minecraftforge/fml/common/discovery/ModCandidate;Ljava/lang/String;Ljava/util/Set;Ljava/util/Set;Lnet/minecraftforge/fml/common/discovery/ASMDataTable;)V", false));
    ...
    private byte[] nbtTagStringRevamp(byte[] bytes) {
        ...
        // node.fields.removeIf(f -> f.name.equals(NormalLoadingPlugin.isDeobf ? "data" : "field_74751_a"));
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals("<init>") && method.desc.equals("(Ljava/lang/String;)V")) {
                ...
                while (iter.hasNext()) {
                    AbstractInsnNode instruction = iter.next();
                    if (instruction.getOpcode() == PUTFIELD) {
                        method.instructions.insertBefore(instruction, new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/core/NormalHooks", "nbtTagString$override$ctor", "(Ljava/lang/String;)Ljava/lang/String;", false));
                    }
                }
            }
        }
    ...
    private byte[] stripItemStackFields(byte[] bytes) {
        ...
        fields[0] = !NormalLoadingPlugin.isDeobf ? "field_82843_f" : "itemFrame";
        fields[1] = !NormalLoadingPlugin.isDeobf ? "field_179552_h" : "canDestroyCacheBlock";
        fields[2] = !NormalLoadingPlugin.isDeobf ? "field_179553_i" : "canDestroyCacheResult";
        fields[3] = !NormalLoadingPlugin.isDeobf ? "field_179550_j" : "canPlaceOnCacheBlock";
        fields[4] = !NormalLoadingPlugin.isDeobf ? "field_179551_k" : "canPlaceOnCacheResult";
    ...
    private byte[] stripLocalsInEnableStandardItemLighting(byte[] bytes) {
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals(NormalLoadingPlugin.isDeobf ? "enableStandardItemLighting" : "func_74519_b")) {
                ListIterator<AbstractInsnNode> iter = method.instructions.iterator();
                while (iter.hasNext()) {
                    AbstractInsnNode instruction = iter.next();
            ...
            if (method.name.equals("<init>") || method.name.equals("setLabel")) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == PUTFIELD && ((FieldInsnNode) instruction).name.equals("label")) {
                        iter.previous();
                        iter.add(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/api/NormalStringPool", "canonicalize", "(Ljava/lang/String;)Ljava/lang/String;", false));
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == PUTFIELD) {
                        ...
                        if (fieldNode.name.equals("annotationName") || fieldNode.name.equals("className")) {
                            iter.previous();
                            iter.add(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/core/NormalHooks", "asmData$redirect$CtorStringsToIntern", "(Ljava/lang/String;)Ljava/lang/String;", false));
                            iter.next();
                        }
                    }
                }
    ...
    private byte[] injectBlastingOilEntityItemUpdate(byte[] bytes) {
        if (!Gameplay.disableBlastingOilEvents) {
            ...
            methodVisitor.visitMethodInsn(INVOKESTATIC, "mirror/normalasm/common/modfixes/betterwithmods/BWMBlastingOilOptimization", "inject$ItemMaterial$onEntityItemUpdate", "(Lnet/minecraft/entity/item/EntityItem;)Z", false);
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == PUTFIELD) {
                        ...
                        if (fieldNode.desc.equals("[I")) {
                            iter.previous();
                            iter.add(new VarInsnNode(ALOAD, 0));
                            iter.add(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/bakedquad/NormalVertexDataPool", "canonicalize", "([ILnet/minecraft/client/renderer/block/model/BakedQuad;)[I", false));
                            method.maxStack = 3;
                            break;
                        }
                    }
                    ...
                    if (instruction.getOpcode() == INVOKESTATIC) {
                        ...
                        switch (methodNode.name) {
                            case "newIdentityHashMap":
                                methodNode.owner = "mirror/normalasm/core/NormalHooks";
    ...
    private byte[] delayItemStackCapabilityInit(byte[] bytes) {
        ...
        String writeToNBT = !NormalLoadingPlugin.isDeobf ? "func_77955_b" : "writeToNBT";
        String isEmpty = !NormalLoadingPlugin.isDeobf ? "func_82582_d" : "isEmpty";
        String setTag = !NormalLoadingPlugin.isDeobf ? "func_74782_a" : "setTag";
    ...
    private byte[] removeExcessiveGCCalls(byte[] bytes) {
        ...
        String loadWorld = !NormalLoadingPlugin.isDeobf ? "func_71353_a" : "loadWorld";
    ...
    private byte[] smoothDimensionChange(byte[] bytes) {
        ...
        String handleJoinGame = !NormalLoadingPlugin.isDeobf ? "func_147282_a" : "handleJoinGame";
        String handleRespawn = !NormalLoadingPlugin.isDeobf ? "func_147280_a" : "handleRespawn";
    ...
    private byte[] fixMC30845(byte[] bytes) {
        ...
        String orientCamera = !NormalLoadingPlugin.isDeobf ? "func_78467_g" : "orientCamera";
        String rayTraceBlocksOld = !NormalLoadingPlugin.isDeobf ? "func_72933_a" : "rayTraceBlocks";
        String rayTraceBlocksNew = !NormalLoadingPlugin.isDeobf ? "func_147447_a" : "rayTraceBlocks";
    ...
    private byte[] fixMC31681(byte[] bytes) {
        ...
        String updateRenderer = !NormalLoadingPlugin.isDeobf ? "func_78464_a" : "updateRenderer";
        String renderDistanceChunks = !NormalLoadingPlugin.isDeobf ? "field_151451_c" : "renderDistanceChunks";
    ...
    private byte[] disappearingEntitiesRenderGlobalFix(byte[] bytes) {
        ...
        String setupTerrain = !NormalLoadingPlugin.isDeobf ? "func_174970_a" : "setupTerrain";
        String renderChunkBoundingBox = !NormalLoadingPlugin.isDeobf ? "field_178591_c" : "boundingBox";
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == GETFIELD) {
                        ...
                        if (getField.name.equals(renderChunkBoundingBox)) {
                            iter.set(new MethodInsnNode(INVOKESTATIC, "mirror/normalasm/patches/RenderGlobalPatch", "getCorrectBoundingBox", "(Lnet/minecraft/client/renderer/chunk/RenderChunk;)Lnet/minecraft/util/math/AxisAlignedBB;", false));
                        }
                    }
                }
            }
